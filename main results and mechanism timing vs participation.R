...
library(xtsum)
pdata = pdata.frame(merged_lpi2, index = c("code", "year"), drop.index = TRUE)
pdata$log_gdp_per_capita = log(pdata$gdp_per_capita)
pdata_sum = xtsum(pdata, variables = c('overall_lpi', 'customs', 'infra', 'logistics_services',
                                       'ease_tracking', 'timeliness', 'log_gdp_per_capita',
                                       'import_pct_lcu', 'export_pct_lcu', 'manufac_pct_lcu',
                                       'corruption_control'),
                  return.data.frame = TRUE)

write_xlsx(pdata_sum, 'Laporan/Tabel Statistik Deskriptif.xlsx')
...
stargazer(twfe_ol_wo, twfe_ol_controls, twfe_customs_wo, twfe_customs_controls, 
          ...
          type = 'html',
          out = 'Laporan/Tabel Regresi Part 1.html', no.space = TRUE, align = TRUE,
...
stargazer(twfe_infra_wo, twfe_infra_controls, twfe_ls_wo, twfe_ls_controls,
          ...
          type = 'html',
          out = 'Laporan/Tabel Regresi Part 2.html', no.space = TRUE, align = TRUE,
          omit.stat=c("LL","ser","f"))
...
stargazer(twfe_et_wo, twfe_et_controls, twfe_timeliness_wo, twfe_timeliness_controls,
          ...
          type = 'html',
          out = 'Laporan/Tabel Regresi Part 3.html', no.space = TRUE, align = TRUE,
          omit.stat=c("LL","ser","f"))
